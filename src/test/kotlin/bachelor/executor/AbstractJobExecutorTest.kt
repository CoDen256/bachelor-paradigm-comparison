...
abstract class AbstractJobExecutorTest(private val createExecutor: (JobApi) -> JobExecutor) {
    ...
    inner class GivenJobSuccessfulyCreated {
        ...
        inner class GivenLateRunningPodEventsAndDelayedWaitingEventsButNoJobEventsAndNoLogs {
            ...
            fun `Given three delayed waiting pod events Then latest waiting snapshot`() {
                podEvents.addAll(listOf(
                    ...
                    // 250ms running timeout
                    // 300ms terminated timeout
                    add(runningPodSnapshot), // 300ms
                ))

                // execute
                val ex = assertThrows<PodNotRunningTimeoutException> {
                    execute(millis(250), millis(300))
                }

                assertEquals(snapshot(pod = waitingSnapshot), ex.currentState)
                assertEquals(millis(250), ex.timeout)
            }
        }
        @Nested
        ...
        inner class GivenLateTerminatedAndDelayedRunningPodEventsButNoJobEventsAndNoLogs {
            ...
            @Test
            fun `Given delayed waiting and late terminated pod event Then the running snapshot`() {
                podEvents.addAll(listOf(
                    add(waitingPodSnapshot), // 0ms
                    // 50ms running timeout
                    // 70ms terminated timeout
                    add(terminatedPodSnapshot) // 100ms
                ))

                // execute
                val ex = assertThrows<PodNotRunningTimeoutException> {
                    execute(millis(50), millis(70))
                }
        ...
        @Nested
        @DisplayName("Given FailedTerminatedPod events and no logs and no job events When executed Then throw PodTerminatedWithErrorException")
        inner class GivenDelayedTerminatedPodEventsButNoJobEventsAndNoLogs {
            ...
            fun setup() {
                podEvents.clear()
                whenever(api.addPodEventHandler(any())).thenWithPodHandler { handler ->
                    emitDelayed(podEvents, handler, 100)
                }
            }
            @AfterEach
            fun teardown(){
            ...
            @Test
            fun `Given failed terminated pod event delayed Then the terminated snapshot`() {
                podEvents.addAll(listOf(
        ...
        inner class GivenSuccessfullyTerminatedPodEventsButNoJobEventsAndNoLogs {
            ...
            private val podRef = running.element!!.reference()
            ...
            fun `Given successfully terminated pod event Then the terminated snapshot`() {
                ...
                assertEquals(snapshot(pod = succeeded.element!!), result)
            }
            ...
            fun `Given running and successfully terminated pod event Then the terminated snapshot`() {
                podEvents {
                    emit(running) // 0ms
                    // 50ms running timeout
                    emit(millis(200), succeeded) // 200ms
                }
            ...
            fun `Given running waiting and successfully terminated pod event Then the terminated snapshot`() {
                ...
                assertEquals(snapshot(pod = waiting.element!!), ex.currentState)
            ...
            fun `Given running directly running and successfully terminated in 200ms pod event Then the terminated snapshot`() {
                podEvents {
                    ...
                    emit(running) // 0ms
                    emit(millis(100), succeeded) // 100ms
            ...
            fun `Given running direct and successfully terminated after 300ms pod event Then the terminated snapshot`() {
                podEvents {
                    ...
                    emit(millis(300), succeeded) // 300ms
                }
                ...
                assertEquals(snapshot(pod = running.element!!), ex.currentState)
                assertEquals(millis(100), ex.timeout)
            }
    ...
    fun podEvents(vararg events: Pair<Int, ResourceEvent<ActivePodSnapshot>>) {
        events.forEach { (delay, element) ->
            podEventEmitter.emit(Duration.ofMillis(delay.toLong()), element)
        }
    }
